...
export const clinicStaffAccounts = mysqlTable("clinic_staff_accounts", {
  id: int("id").autoincrement().primaryKey(), clinicianUserId: int("clinicianUserId").notNull(), staffAccountId: varchar("staffAccountId", { length: 120 }).notNull(), invitedEmail: varchar("invitedEmail", { length: 320 }).notNull(), staffUserId: int("staffUserId"), displayName: varchar("displayName", { length: 255 }), staffRole: mysqlEnum("staffRole", ["receptionist", "nurse", "clinician"]).notNull(), status: mysqlEnum("status", ["invited", "active", "revoked"]).notNull().default("invited"), invitedBy: varchar("invitedBy", { length: 255 }).notNull(), invitedAt: timestamp("invitedAt").defaultNow().notNull(), activatedAt: timestamp("activatedAt"), revokedAt: timestamp("revokedAt"), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
