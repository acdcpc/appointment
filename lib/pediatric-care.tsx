...
type PediatricCareContextValue = {
  children: ChildProfile[]; activeChild: ChildProfile; appointments: PediatricAppointment[]; prescriptions: PrescriptionRecord[]; history: MedicalHistoryEntry[]; clinicHours: ClinicOperatingHour[]; clinicBreaks: ClinicBreak[]; clinicHolidays: ClinicHoliday[]; services: ServiceConfig[];
...
const initialAppointments: PediatricAppointment[] = [{ id: "apt-1", childId: "child-1", service: "Pediatric consultation", date: "Tue, Aug 20", time: "3:30 PM", durationMinutes: 30, reason: "Well-child follow-up", status: "confirmed" }, { id: "apt-2", childId: "child-1", service: "Child development review", date: "Fri, Aug 30", time: "10:00 AM", durationMinutes: 45, reason: "Developmental milestone discussion", status: "needs-intake" }];
...
export function PediatricCareProvider({ children: content }: { children: ReactNode }) {
  ...
  const value = useMemo<PediatricCareContextValue>(() => {
    ...
    return {
      children, activeChild: children[0], appointments, prescriptions, history, clinicHours, clinicBreaks, clinicHolidays, services, getAvailableSlots,
  ...
  }, [appointments, clinicBreaks, clinicHolidays, clinicHours, prescriptions, services]);
