import { createContext, type ReactNode, useContext, useEffect, useMemo, useState } from "react";
import { getSupabase, getSupabaseSession } from "./supabase";
import { ageInMonths, describeMeasurement, normaliseDraft, validateGrowthMeasurement, type GrowthMeasurementInput } from "./growth-measurements";

export type ChildProfile = { id: string; name: string; dateOfBirth: string; sex: "male" | "female"; allergies: string; parentName: string };
export type AppointmentStatus = "confirmed" | "needs-intake" | "completed" | "cancelled";
export type PediatricAppointment = { id: string; childId: string; service: string; date: string; time: string; durationMinutes: number; reason: string; status: AppointmentStatus; changeMessage?: string; guardianConfirmedAt?: string; rescheduledAt?: string; rescheduleAcknowledgedAt?: string; appointmentChangeReminderDraftedAt?: string };
export type PrescriptionRecord = { id: string; childId: string; appointmentId: string; issuedOn: string; medication: string; instructions: string; status: "active" | "completed" };
export type MedicalHistoryEntry = { id: string; childId: string; category: "Allergy" | "Development" | "Visit" | "Immunization" | "Communication"; title: string; occurredOn: string; note: string };
export type GrowthSaveResult = { ok: true; message: string } | { ok: false; message: string };
export type GrowthMetric = { id: string; childId: string; occurredOn: string; ageMonths: number; weightKg?: number; heightCm?: number; headCircumferenceCm?: number; note: string; recordedRole?: "clinic" | "guardian"; recordedBy?: string };
export type ClinicOperatingHour = { weekday: "Mon" | "Tue" | "Wed" | "Thu" | "Fri" | "Sat" | "Sun"; label: string; start: string; end: string; isOpen: boolean };
export type ClinicBreak = { id: string; weekday: ClinicOperatingHour["weekday"]; start: string; end: string };
export type ClinicHoliday = { id: string; date: string; label: string };
export type ClinicDayHourOverride = { overrideId: string; appointmentDate: string; isOpen: boolean; startTime?: string | null; endTime?: string | null; familyNotice: string; updatedAt?: string };
export type ServiceConfig = { name: string; durationMinutes: number; waitlistOfferDurationHours?: number };
export type ServicePreparationChecklist = { service: string; items: string[]; updatedAt: string };
export type EarlierSlotOffer = { date: string; time: string; offeredAt: string; expiresAt: string; deliveryStatus?: EmailShareStatus; parentResponse?: "accepted" | "declined"; respondedAt?: string; clinicianAcknowledgedAt?: string; convertedAt?: string; convertedBy?: string };
export type EarlierSlotRequest = { id: string; appointmentId: string; childId: string; requestedAt: string; status: "pending" | "reviewed" | "declined" | "withdrawn" | "offered" | "responded" | "expired" | "converted"; note?: string; offer?: EarlierSlotOffer; assignedStaffId?: string; assignedAt?: string };
export type ReferralService = { id: string; name: string; recipient: string; organization: string; email: string; defaultPurpose: string; approvalStatus: "pending" | "approved" | "rejected"; decisionBy?: string; decisionAt?: string };
export type EmailShareStatus = "draft-opened" | "sent" | "saved" | "cancelled" | "unavailable";
export type PatientCommunicationScope = "message" | "record-pdf" | "timeline-report";
export type PatientAuditEvent = { id: string; childId: string; type: "appointment-change" | "referral-letter" | "email-share" | "patient-communication"; occurredOn: string; actorRole: "clinician"; actorName?: string; summary: string; message?: string; deliveryStatus?: EmailShareStatus; isResend?: boolean; recordedAt?: string; attemptedAt?: string; archivedAt?: string; archivedBy?: string; archiveReason?: string; serverRetryLimit?: number; serverRetryAttempts?: number };
export type ReferralLetterSettings = { clinicName: string; clinicContact: string; signatureName: string; signatureTitle: string };
export type ClinicLocationSettings = { address: string; mapUrl: string; clinicEmail: string; isProvisional: boolean };
export type StaffRole = "receptionist" | "nurse" | "clinician";
export type StaffMember = { id: string; name: string; role: StaffRole; triageCapacity?: number; triageCapacityUpdatedAt?: string };
export type StaffInvitation = { id: string; email: string; role: StaffRole; status: "pending" };
export const auditEventTypesByRole: Record<StaffRole, PatientAuditEvent["type"][]> = { receptionist: ["appointment-change"], nurse: ["appointment-change", "referral-letter"], clinician: ["appointment-change", "referral-letter", "email-share", "patient-communication"] };
export const MAX_REFERRAL_EMAIL_RESENDS = 3;
export function countReferralEmailResends(events: PatientAuditEvent[], childId: string, email: string) { return events.filter((event) => event.type === "email-share" && event.isResend && event.childId === childId && event.message?.toLowerCase() === email.trim().toLowerCase()).length; }
export function countUnresolvedReferralDeliveryFailures(events: PatientAuditEvent[], childId: string) { return events.filter((event) => event.type === "email-share" && event.childId === childId && (event.deliveryStatus === "cancelled" || event.deliveryStatus === "unavailable")).length; }
export function referralEmailAttemptTimestamp(event: PatientAuditEvent) { return event.attemptedAt ?? event.recordedAt ?? event.occurredOn; }
export type AuditStaffActionFilter = "all" | "appointment-change" | "referral-letter" | "email-share" | "patient-communication" | "resend";
export function filterAuditEventsForExport(events: PatientAuditEvent[], childId: string, options: { type?: "all" | PatientAuditEvent["type"]; staffAction?: AuditStaffActionFilter; startDate?: string; endDate?: string }) {
  const start = options.startDate ? new Date(`${options.startDate}T00:00:00.000Z`).getTime() : Number.NEGATIVE_INFINITY;
  const end = options.endDate ? new Date(`${options.endDate}T23:59:59.999Z`).getTime() : Number.POSITIVE_INFINITY;
  return events.filter((event) => { const timestamp = event.attemptedAt ?? event.recordedAt; const instant = timestamp ? new Date(timestamp).getTime() : Number.NaN; const actionMatches = !options.staffAction || options.staffAction === "all" || event.type === options.staffAction || (options.staffAction === "resend" && event.isResend); return event.childId === childId && (!options.type || options.type === "all" || event.type === options.type) && actionMatches && (!timestamp || (Number.isFinite(instant) && instant >= start && instant <= end)); });
}

type BookingInput = Pick<PediatricAppointment, "childId" | "service" | "date" | "time" | "reason">;
type PrescriptionInput = Pick<PrescriptionRecord, "childId" | "appointmentId" | "medication" | "instructions">;
type Result = { ok: true } | { ok: false; message: string };
type PediatricCareContextValue = {
  children: ChildProfile[]; childrenSource: "demo" | "server"; pendingReports: Array<{ id: number; childId: string; scope: string; token: string }>; activeChild: ChildProfile; setActiveChild: (childId: string) => void; appointments: PediatricAppointment[]; replaceAppointments: (appointments: PediatricAppointment[]) => void; prescriptions: PrescriptionRecord[]; history: MedicalHistoryEntry[]; growthMetrics: GrowthMetric[]; recordGrowthMeasurement: (input: GrowthMeasurementInput) => Promise<GrowthSaveResult>; clinicHours: ClinicOperatingHour[]; clinicBreaks: ClinicBreak[]; clinicHolidays: ClinicHoliday[]; clinicDayHourOverrides: ClinicDayHourOverride[]; replaceClinicDayHourOverrides: (overrides: ClinicDayHourOverride[]) => void; services: ServiceConfig[]; preparationChecklists: ServicePreparationChecklist[]; earlierSlotRequests: EarlierSlotRequest[]; replaceEarlierSlotRequests: (requests: EarlierSlotRequest[]) => void; waitlistOfferDurationHours: number; referralServices: ReferralService[]; auditEvents: PatientAuditEvent[]; replaceAuditEvents: (events: PatientAuditEvent[]) => void; referralLetterSettings: ReferralLetterSettings; clinicLocation: ClinicLocationSettings; staffMembers: StaffMember[]; applyStaffCapacitySnapshots: (capacities: Array<{ staffId: string; triageCapacity: number }>) => void; staffInvitations: StaffInvitation[];
  updateChildProfile: (childId: string, changes: Partial<Pick<ChildProfile, "name" | "allergies">>) => Promise<Result>; getAvailableSlots: (date: string, service: string, omitAppointmentId?: string) => string[]; bookAppointment: (input: BookingInput) => Result; rescheduleAppointment: (appointmentId: string, date: string, time: string, message?: string) => Result; cancelAppointment: (appointmentId: string, message?: string) => Result; confirmGuardianAttendance: (appointmentId: string) => Result; acknowledgeAppointmentChange: (appointmentId: string) => Result; recordAppointmentChangeReminderDraft: (appointmentId: string) => Result; updateClinicHour: (weekday: ClinicOperatingHour["weekday"], changes: Partial<ClinicOperatingHour>) => void; updateServiceDuration: (service: string, durationMinutes: number) => void; updateServicePreparation: (service: string, items: string[]) => Result; updateWaitlistOfferDuration: (hours: number) => Result; updateServiceWaitlistOfferDuration: (service: string, hours: number | undefined) => Result; requestEarlierSlot: (appointmentId: string, note?: string) => Result; withdrawEarlierSlotRequest: (id: string) => Result; createEarlierSlotOffer: (id: string, date: string, time: string) => Result; respondToEarlierSlotOffer: (id: string, response: "accepted" | "declined") => Result; convertAcceptedEarlierSlotOffer: (id: string) => Result; acknowledgeEarlierSlotResponse: (id: string) => void; assignEarlierSlotRequest: (id: string, staffId?: string) => Result; updateStaffTriageCapacity: (id: string, capacity: number) => Result; reviewEarlierSlotRequest: (id: string, status: "reviewed" | "declined") => void; addClinicBreak: (weekday: ClinicBreak["weekday"], start: string, end: string) => Result; removeClinicBreak: (id: string) => void; addClinicHoliday: (date: string, label: string) => Result; removeClinicHoliday: (id: string) => void; addReferralService: (input: Omit<ReferralService, "id" | "approvalStatus" | "decisionBy" | "decisionAt">) => Result; approveReferralService: (id: string) => void; rejectReferralService: (id: string) => void; removeReferralService: (id: string) => void; updateReferralLetterSettings: (settings: ReferralLetterSettings) => void; updateClinicLocation: (settings: ClinicLocationSettings) => void; addStaffMember: (name: string, role: StaffRole) => Result; updateStaffRole: (id: string, role: StaffRole) => void; removeStaffMember: (id: string) => void; inviteStaffMember: (email: string, role: StaffRole) => Result; cancelStaffInvitation: (id: string) => void; recordReferralLetter: (childId: string, recipient: string, purpose: string) => void; recordReferralEmailShare: (childId: string, recipient: string, email: string, status: EmailShareStatus, isResend?: boolean, metadata?: Pick<PatientAuditEvent, "attemptedAt" | "serverRetryLimit" | "serverRetryAttempts">) => void; recordPatientCommunication: (childId: string, recipient: string, email: string, scope: PatientCommunicationScope, status: EmailShareStatus) => void; recordReminderDraft: (childId: string, recipient: string, reportLabel: string) => void; writePrescription: (input: PrescriptionInput) => Result;
};

const initialChildProfiles: ChildProfile[] = [
  { id: "child-1", name: "Aarav Smith", dateOfBirth: "14 May 2022", sex: "male", allergies: "No known drug allergies reported", parentName: "Jordan Smith" },
  { id: "child-2", name: "Maya Gurung", dateOfBirth: "03 September 2020", sex: "female", allergies: "Parent reports no known allergies", parentName: "Nisha Gurung" },
  { id: "child-3", name: "Rohan Thapa", dateOfBirth: "21 January 2019", sex: "male", allergies: "See parent-provided allergy history", parentName: "Suman Thapa" },
];
const APPOINTMENTS_KEY = "rainbow-appointments";
/** Booked visits were React state only, so a booked visit vanished on reload. */
function loadPersistedAppointments(): PediatricAppointment[] | null {
  try {
    if (typeof window === "undefined" || !window.localStorage) return null;
    const raw = window.localStorage.getItem(APPOINTMENTS_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as PediatricAppointment[];
    return Array.isArray(parsed) ? parsed : null;
  } catch { return null; }
}
function persistAppointments(list: PediatricAppointment[]) {
  try {
    if (typeof window !== "undefined" && window.localStorage) window.localStorage.setItem(APPOINTMENTS_KEY, JSON.stringify(list));
  } catch { /* ignore quota / private-mode storage errors */ }
}
const initialHours: ClinicOperatingHour[] = [
  { weekday: "Mon", label: "Monday", start: "17:30", end: "20:00", isOpen: true }, { weekday: "Tue", label: "Tuesday", start: "17:30", end: "20:00", isOpen: true }, { weekday: "Wed", label: "Wednesday", start: "11:00", end: "17:00", isOpen: true }, { weekday: "Thu", label: "Thursday", start: "17:30", end: "20:00", isOpen: true }, { weekday: "Fri", label: "Friday", start: "17:30", end: "20:00", isOpen: true }, { weekday: "Sat", label: "Saturday", start: "17:00", end: "19:00", isOpen: true }, { weekday: "Sun", label: "Sunday", start: "17:30", end: "20:00", isOpen: true },
];
const initialServices: ServiceConfig[] = [{ name: "Pediatric consultation", durationMinutes: 30 }, { name: "Child development review", durationMinutes: 45 }, { name: "Growth & wellbeing", durationMinutes: 30 }];
const initialPreparationChecklists: ServicePreparationChecklist[] = [
  { service: "Pediatric consultation", items: ["Bring any recent prescriptions or reports.", "Write down questions or concerns for Dr. Ojha."], updatedAt: "2026-08-20T00:00:00.000Z" },
  { service: "Child development review", items: ["Bring school, therapy, or prior developmental reports if available.", "Note recent milestones, concerns, and questions."], updatedAt: "2026-08-20T00:00:00.000Z" },
  { service: "Growth & wellbeing", items: ["Bring recent growth records or nutrition notes if available.", "Note any feeding, sleep, or wellbeing questions."], updatedAt: "2026-08-20T00:00:00.000Z" },
];
const initialAppointments: PediatricAppointment[] = [];
const initialPrescriptions: PrescriptionRecord[] = [];
const initialHistory: MedicalHistoryEntry[] = [
  { id: "history-1", childId: "child-1", category: "Allergy", title: "Allergy record", occurredOn: "12 Aug 2026", note: "No known drug allergies reported by parent." }, { id: "history-2", childId: "child-1", category: "Development", title: "Development review requested", occurredOn: "05 Aug 2026", note: "Parent requested a review of developmental milestones at the next visit." }, { id: "history-3", childId: "child-1", category: "Visit", title: "Pediatric follow-up", occurredOn: "20 Jul 2026", note: "Visit summary available in the clinical record." },
  { id: "history-4", childId: "child-2", category: "Visit", title: "Respiratory review requested", occurredOn: "14 Aug 2026", note: "Parent requested a clinician review after a recent cough." }, { id: "history-5", childId: "child-3", category: "Development", title: "Growth and wellbeing review", occurredOn: "10 Aug 2026", note: "Parent requested nutrition and growth discussion." },
];
const demoGrowthMetrics: GrowthMetric[] = [
  { id: "growth-1", childId: "child-1", occurredOn: "12 Aug 2026", ageMonths: 51, weightKg: 15.2, heightCm: 99.4, note: "Measurement recorded at pediatric follow-up." },
  { id: "growth-2", childId: "child-1", occurredOn: "20 Jul 2026", ageMonths: 50, weightKg: 14.9, heightCm: 98.7, note: "Measurement recorded during a prior visit." },
  { id: "growth-3", childId: "child-2", occurredOn: "14 Aug 2026", ageMonths: 71, weightKg: 17.4, heightCm: 106.1, note: "Parent-visible clinic measurement record." },
  { id: "growth-4", childId: "child-3", occurredOn: "10 Aug 2026", ageMonths: 91, weightKg: 20.6, heightCm: 116.8, note: "Growth and wellbeing visit measurement." },
];

/** Maps a stored measurement row (snake_case columns) into the chart shape. */
function mapGrowthRow(row: Record<string, unknown>): GrowthMetric {
  const optionalNumber = (value: unknown) => (value === null || value === undefined || value === "" ? undefined : Number(value));
  return {
    id: String(row.id ?? ""),
    childId: String(row.child_id ?? row.childId ?? ""),
    occurredOn: String(row.measured_on ?? row.measuredOn ?? ""),
    ageMonths: Number(row.age_months ?? row.ageMonths ?? 0),
    weightKg: optionalNumber(row.weight_kg ?? row.weightKg),
    heightCm: optionalNumber(row.height_cm ?? row.heightCm),
    headCircumferenceCm: optionalNumber(row.head_circumference_cm ?? row.headCircumferenceCm),
    note: String(row.note ?? ""),
    recordedRole: String(row.recorded_role ?? "clinic") === "guardian" ? "guardian" : "clinic",
    recordedBy: row.recorded_by === null || row.recorded_by === undefined ? undefined : String(row.recorded_by),
  };
}

const timeToMinutes = (time: string) => { const [clock, suffix = ""] = time.trim().split(" "); const [hourString, minuteString] = clock.split(":"); let hours = Number(hourString); const minutes = Number(minuteString); if (!Number.isFinite(hours) || !Number.isFinite(minutes) || minutes < 0 || minutes > 59) return Number.NaN; if (/(AM|PM)/i.test(suffix)) { if (suffix.toUpperCase() === "PM" && hours !== 12) hours += 12; if (suffix.toUpperCase() === "AM" && hours === 12) hours = 0; } return hours * 60 + minutes; };
const minutesToDisplay = (minutes: number) => { const hour = Math.floor(minutes / 60); const minute = minutes % 60; const suffix = hour >= 12 ? "PM" : "AM"; const displayHour = hour % 12 || 12; return `${displayHour}:${minute.toString().padStart(2, "0")} ${suffix}`; };
const weekdayFromDate = (date: string) => {
  if (/^\d{4}-\d{2}-\d{2}$/.test(date)) return ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][new Date(`${date}T12:00:00`).getDay()] as ClinicOperatingHour["weekday"];
  return date.slice(0, 3) as ClinicOperatingHour["weekday"];
};
const overlap = (startA: number, endA: number, startB: number, endB: number) => startA < endB && endA > startB;
export function findAppointmentConflicts(appointments: PediatricAppointment[]) {
  const conflicts = new Map<string, PediatricAppointment[]>();
  appointments.filter((item) => item.status !== "completed" && item.status !== "cancelled").forEach((appointment) => {
    const start = timeToMinutes(appointment.time); const end = start + appointment.durationMinutes;
    const matches = appointments.filter((other) => other.id !== appointment.id && other.status !== "completed" && other.status !== "cancelled" && other.date === appointment.date && overlap(start, end, timeToMinutes(other.time), timeToMinutes(other.time) + other.durationMinutes));
    if (matches.length) conflicts.set(appointment.id, matches);
  });
  return conflicts;
}
const validRange = (start: string, end: string) => { const from = timeToMinutes(start); const to = timeToMinutes(end); return Number.isFinite(from) && Number.isFinite(to) && from < to; };
const PediatricCareContext = createContext<PediatricCareContextValue | null>(null);

export function PediatricCareProvider({ children: content }: { children: ReactNode }) {
  const [childrenSource, setChildrenSource] = useState<"demo" | "server">("demo");
  const [pendingReports, setPendingReports] = useState<Array<{ id: number; childId: string; scope: string; token: string }>>([]);
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const session = await getSupabaseSession();
        const client = getSupabase();
        if (!session || !client) return;
        const { data, error } = await client.from("clinic_children").select("*");
        if (cancelled || error || !data?.length) return;
        const mapped = data.map((row) => ({
          id: String(row.childId ?? ""),
          name: String(row.name ?? ""),
          dateOfBirth: String(row.dateOfBirth ?? ""),
          sex: String(row.sex ?? "male") === "female" ? "female" as const : "male" as const,
          allergies: String(row.allergies ?? ""),
          parentName: String(row.parentName ?? ""),
        })).filter((child) => child.id);
        if (cancelled || !mapped.length) return;
        setChildProfiles(mapped);
        setChildrenSource("server");
        // Drop any prototype-era local copy so a renamed child can never be
        // resurrected from this browser's storage.
        try { if (typeof window !== "undefined" && window.localStorage) window.localStorage.removeItem("rainbow-child-profiles"); } catch { /* ignore */ }
        // Reports the clinic published for these children and the guardian has
        // not yet acknowledged — surfaced on Home so nothing has to be
        // remembered. Read through the existing guardian RLS policy.
        try {
          const { data: shareRows } = await client
            .from("patient_report_shares")
            .select("id,childId,scope,acknowledgementToken,acknowledgedAt")
            .is("acknowledgedAt", null);
          if (!cancelled && shareRows?.length) {
            setPendingReports(shareRows.map((row) => ({
              id: Number(row.id),
              childId: String(row.childId ?? ""),
              scope: String(row.scope ?? "record-pdf"),
              token: String(row.acknowledgementToken ?? ""),
            })).filter((row) => row.token));
          }
        } catch { /* no published reports is a normal state */ }
      } catch { /* keep demo children on any failure */ }
    })();
    return () => { cancelled = true; };
  }, []);
  const [childProfiles, setChildProfiles] = useState<ChildProfile[]>(() => {
    // Persist guardian-edited child details across reloads (web localStorage;
    // native keeps the in-memory demo defaults). Prototype-scoped by design.
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        const stored = window.localStorage.getItem("rainbow-child-profiles");
        if (stored) {
          const parsed = JSON.parse(stored) as ChildProfile[];
          if (Array.isArray(parsed) && parsed.length) return parsed;
        }
      }
    } catch { /* ignore storage errors */ }
    return initialChildProfiles;
  });
  const [selectedChildId, setSelectedChildId] = useState(initialChildProfiles[0].id); const [appointments, setAppointments] = useState<PediatricAppointment[]>(() => loadPersistedAppointments() ?? initialAppointments); const [clinicHours, setClinicHours] = useState<ClinicOperatingHour[]>(initialHours); const [clinicBreaks, setClinicBreaks] = useState<ClinicBreak[]>([]); const [clinicHolidays, setClinicHolidays] = useState<ClinicHoliday[]>([]); const [clinicDayHourOverrides, setClinicDayHourOverrides] = useState<ClinicDayHourOverride[]>([]); const [services, setServices] = useState<ServiceConfig[]>(initialServices); const [preparationChecklists, setPreparationChecklists] = useState<ServicePreparationChecklist[]>(initialPreparationChecklists); const [earlierSlotRequests, setEarlierSlotRequests] = useState<EarlierSlotRequest[]>([]); const [waitlistOfferDurationHours, setWaitlistOfferDurationHours] = useState(24); const [prescriptions, setPrescriptions] = useState<PrescriptionRecord[]>(initialPrescriptions); const [history, setHistory] = useState<MedicalHistoryEntry[]>(initialHistory); const [referralServices, setReferralServices] = useState<ReferralService[]>([]); const [auditEvents, setAuditEvents] = useState<PatientAuditEvent[]>([]); const [referralLetterSettings, setReferralLetterSettings] = useState<ReferralLetterSettings>({ clinicName: "Rainbow Child Development Clinic", clinicContact: "Phone: 9765002862 · rainbowclinic25@gmail.com", signatureName: "Associate Professor Dr. Anil Ojha, MBBS, MD, FCCH", signatureTitle: "Developmental Pediatrician" }); const [clinicLocation, setClinicLocation] = useState<ClinicLocationSettings>({ address: "Gokul Awas Rd, Karyabinayak 44700", mapUrl: "https://www.google.com.au/search?client=safari&hs=ORpV&sca_esv=79a7fd24df7232ff&hl=en-au&kgmid=/g/11zhz76ycx&q=Rainbow+Child+Development+Clinic&shem=epsd1,ltae,rimspwouoe&shndl=30&source=sh/x/loc/act/m1/3&kgs=21ca31c1d4d885a7&utm_source=epsd1,ltae,rimspwouoe,sh/x/loc/act/m1/3", clinicEmail: "rainbowclinic25@gmail.com", isProvisional: false }); const [staffMembers, setStaffMembers] = useState<StaffMember[]>([{ id: "staff-ojha", name: "Associate Professor Dr. Anil Ojha, MBBS, MD, FCCH", role: "clinician" }]); const [staffInvitations, setStaffInvitations] = useState<StaffInvitation[]>([]);
  // Growth measurements are state now (they were a frozen demo list), so a value
  // recorded in the dashboard appears immediately and survives a reload.
  const [growthMetricRows, setGrowthMetricRows] = useState<GrowthMetric[]>(demoGrowthMetrics);
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const session = await getSupabaseSession();
        const client = getSupabase();
        if (!session || !client) return;
        const { data, error } = await client.from("child_growth_measurements").select("*");
        if (cancelled || error || !data) return;
        setGrowthMetricRows(data.map(mapGrowthRow).filter((row) => row.childId));
      } catch { /* keep the prototype values if storage is unreachable */ }
    })();
    return () => { cancelled = true; };
  }, []);

  // Single write-through point: covers booking, reschedule, cancel and attendance.
  useEffect(() => { persistAppointments(appointments); }, [appointments]);
  const value = useMemo<PediatricCareContextValue>(() => {
    const durationFor = (service: string) => services.find((item) => item.name === service)?.durationMinutes ?? 30;
    const getAvailableSlots = (date: string, service: string, omitAppointmentId?: string) => {
      if (clinicHolidays.some((holiday) => holiday.date === date)) return [];
      const day = clinicHours.find((item) => item.weekday === weekdayFromDate(date)); const override = clinicDayHourOverrides.find((item) => item.appointmentDate === date); if ((!override && (!day?.isOpen || !day)) || (override && (!override.isOpen || !override.startTime || !override.endTime))) return [];
      const startValue = override?.startTime ?? day!.start; const endValue = override?.endTime ?? day!.end; if (!validRange(startValue, endValue)) return [];
      const duration = durationFor(service); const start = timeToMinutes(startValue); const end = timeToMinutes(endValue); const dayBreaks = clinicBreaks.filter((item) => item.weekday === day!.weekday && validRange(item.start, item.end)); const slots: string[] = [];
      for (let candidate = start; candidate + duration <= end; candidate += 30) { const blockedByAppointment = appointments.some((appointment) => appointment.id !== omitAppointmentId && appointment.date === date && appointment.status !== "completed" && appointment.status !== "cancelled" && overlap(candidate, candidate + duration, timeToMinutes(appointment.time), timeToMinutes(appointment.time) + appointment.durationMinutes)); const blockedByBreak = dayBreaks.some((breakTime) => overlap(candidate, candidate + duration, timeToMinutes(breakTime.start), timeToMinutes(breakTime.end))); if (!blockedByAppointment && !blockedByBreak) slots.push(minutesToDisplay(candidate)); }
      return slots;
    };
    const recordGrowthMeasurement = async (input: GrowthMeasurementInput): Promise<GrowthSaveResult> => {
      const draft = normaliseDraft(input);
      const validation = validateGrowthMeasurement(draft);
      if (!validation.ok) return { ok: false, message: validation.message };
      const child = childProfiles.find((item) => item.id === draft.childId);
      const months = ageInMonths(child?.dateOfBirth ?? "", draft.measuredOn);
      const localRow: GrowthMetric = {
        id: `growth-${Date.now()}`,
        childId: draft.childId,
        occurredOn: draft.measuredOn,
        ageMonths: months ?? 0,
        weightKg: draft.weightKg,
        heightCm: draft.heightCm,
        headCircumferenceCm: draft.headCircumferenceCm,
        note: draft.note ?? "Measurement recorded in the clinic dashboard.",
        recordedRole: "clinic",
      };
      setGrowthMetricRows((current) => [localRow, ...current]);
      const summary = describeMeasurement(draft, months);
      try {
        const session = await getSupabaseSession();
        const client = getSupabase();
        if (session && client) {
          const { error } = await client.from("child_growth_measurements").insert({
            child_id: draft.childId,
            measured_on: draft.measuredOn,
            age_months: months,
            weight_kg: draft.weightKg ?? null,
            height_cm: draft.heightCm ?? null,
            head_circumference_cm: draft.headCircumferenceCm ?? null,
            note: draft.note ?? null,
            recorded_by: session.user.email ?? null,
            recorded_role: "clinic",
          });
          if (error) return { ok: true, message: `Recorded ${summary} on this device, but it could not be saved to the clinic record. Check the connection and record it again.` };
        }
        return { ok: true, message: `Recorded ${summary}.` };
      } catch {
        return { ok: true, message: `Recorded ${summary} on this device only.` };
      }
    };
    const validateSlot = (date: string, time: string, service: string, omitAppointmentId?: string): Result => getAvailableSlots(date, service, omitAppointmentId).includes(time) ? { ok: true } : { ok: false, message: "That time is outside active clinic hours, overlaps a break or holiday, or no longer has enough time for this visit." };
    const recordAuditEvent = (event: PatientAuditEvent) => { const recordedAt = event.recordedAt ?? new Date().toISOString(); setAuditEvents((events) => [{ ...event, recordedAt }, ...events]); };
    const assignEarlierSlotRequest = (id: string, staffId?: string): Result => {
      const request = earlierSlotRequests.find((item) => item.id === id);
      const isOpen = request?.status === "pending" || (request?.status === "responded" && Boolean(request.offer?.parentResponse) && !request.offer?.clinicianAcknowledgedAt);
      if (!request || !isOpen) return { ok: false, message: "Only open waitlist items can be assigned." };
      if (staffId && !staffMembers.some((staff) => staff.id === staffId)) return { ok: false, message: "Choose an active clinic staff member." };
      setEarlierSlotRequests((current) => current.map((item) => item.id === id ? { ...item, assignedStaffId: staffId, assignedAt: staffId ? new Date().toISOString() : undefined } : item));
      return { ok: true };
    };
    const updateStaffTriageCapacity = (id: string, capacity: number): Result => {
      if (!Number.isInteger(capacity) || capacity < 1 || capacity > 30) return { ok: false, message: "Choose a whole-number triage capacity between 1 and 30." };
      if (!staffMembers.some((staff) => staff.id === id)) return { ok: false, message: "That staff member is no longer active." };
      setStaffMembers((current) => current.map((staff) => staff.id === id ? { ...staff, triageCapacity: capacity, triageCapacityUpdatedAt: new Date().toISOString() } : staff));
      return { ok: true };
    };
    const updateChildProfile = async (childId: string, changes: Partial<Pick<ChildProfile, "name" | "allergies">>): Promise<Result> => {
      const name = changes.name?.trim();
      if (changes.name !== undefined && (!name || name.length < 2)) return { ok: false, message: "Enter the child's full name (at least 2 characters)." };
      if (childrenSource === "server") {
        const client = getSupabase();
        if (!client) return { ok: false, message: "Supabase is not configured on this build." };
        const { error } = await client.from("clinic_children")
          .update({ name: name ?? undefined, allergies: changes.allergies ?? undefined, updatedAt: new Date().toISOString() })
          .eq("childId", childId);
        if (error) return { ok: false, message: error.message };
        setChildProfiles((current) => current.map((child) => child.id === childId ? { ...child, ...changes, ...(name ? { name } : {}) } : child));
        return { ok: true };
      }
      setChildProfiles((current) => {
        const updated = current.map((child) => child.id === childId ? { ...child, ...changes, ...(name ? { name } : {}) } : child);
        try {
          if (typeof window !== "undefined" && window.localStorage) {
            window.localStorage.setItem("rainbow-child-profiles", JSON.stringify(updated));
          }
        } catch { /* ignore storage errors */ }
        return updated;
      });
      return { ok: true };
    };
    return {
      assignEarlierSlotRequest, updateStaffTriageCapacity, updateChildProfile,
      childrenSource,
      pendingReports,
      recordGrowthMeasurement,
      children: childProfiles, activeChild: childProfiles.find((child) => child.id === selectedChildId) ?? childProfiles[0], setActiveChild: setSelectedChildId, appointments, replaceAppointments: setAppointments, prescriptions, history, growthMetrics: growthMetricRows, clinicHours, clinicBreaks, clinicHolidays, clinicDayHourOverrides, replaceClinicDayHourOverrides: setClinicDayHourOverrides, services, preparationChecklists, earlierSlotRequests, replaceEarlierSlotRequests: (persisted) => setEarlierSlotRequests((current) => { const byId = new Map(current.map((request) => [request.id, request])); persisted.forEach((request) => byId.set(request.id, request)); return [...byId.values()].sort((left, right) => Date.parse(right.requestedAt) - Date.parse(left.requestedAt)); }), waitlistOfferDurationHours, referralServices, auditEvents, replaceAuditEvents: (persisted) => setAuditEvents((current) => { const byId = new Map(current.map((event) => [event.id, event])); persisted.forEach((event) => byId.set(event.id, event)); const merged = [...byId.values()].sort((left, right) => Date.parse(right.attemptedAt ?? right.recordedAt ?? "") - Date.parse(left.attemptedAt ?? left.recordedAt ?? "")); return current.length === merged.length && current.every((event, index) => event === merged[index]) ? current : merged; }), referralLetterSettings, clinicLocation, staffMembers, applyStaffCapacitySnapshots: (capacities) => setStaffMembers((current) => current.map((staff) => { const matching = capacities.find((snapshot) => snapshot.staffId === staff.id); return matching ? { ...staff, triageCapacity: matching.triageCapacity } : staff; })), staffInvitations, getAvailableSlots,
      bookAppointment: (input) => { const valid = validateSlot(input.date, input.time, input.service); if (!valid.ok) return valid; const appointment: PediatricAppointment = { id: `apt-${Date.now()}`, ...input, durationMinutes: durationFor(input.service), status: "needs-intake" }; setAppointments((current) => [appointment, ...current]); return { ok: true }; },
      rescheduleAppointment: (appointmentId, date, time, message = "") => { const current = appointments.find((appointment) => appointment.id === appointmentId); if (!current) return { ok: false, message: "The appointment could not be found." }; const valid = validateSlot(date, time, current.service, appointmentId); if (!valid.ok) return valid; setAppointments((items) => items.map((appointment) => appointment.id === appointmentId ? { ...appointment, date, time, status: "confirmed", changeMessage: message.trim() || undefined, rescheduledAt: new Date().toISOString(), rescheduleAcknowledgedAt: undefined, appointmentChangeReminderDraftedAt: undefined } : appointment)); recordAuditEvent({ id: `audit-${Date.now()}`, childId: current.childId, type: "appointment-change", occurredOn: "Today", actorRole: "clinician", actorName: "Associate Professor Dr. Anil Ojha", summary: `Appointment rescheduled to ${date} at ${time}.`, message: message.trim() || undefined }); return { ok: true }; },
      cancelAppointment: (appointmentId, message = "") => { const current = appointments.find((appointment) => appointment.id === appointmentId); if (!current) return { ok: false, message: "The appointment could not be found." }; setAppointments((items) => items.map((appointment) => appointment.id === appointmentId ? { ...appointment, status: "cancelled", changeMessage: message.trim() || undefined } : appointment)); recordAuditEvent({ id: `audit-${Date.now()}`, childId: current.childId, type: "appointment-change", occurredOn: "Today", actorRole: "clinician", actorName: "Associate Professor Dr. Anil Ojha", summary: "Appointment cancelled.", message: message.trim() || undefined }); return { ok: true }; },
      confirmGuardianAttendance: (appointmentId) => { const current = appointments.find((appointment) => appointment.id === appointmentId); if (!current || current.status === "cancelled" || current.status === "completed") return { ok: false, message: "This appointment is no longer available to confirm." }; setAppointments((items) => items.map((appointment) => appointment.id === appointmentId ? { ...appointment, guardianConfirmedAt: new Date().toISOString() } : appointment)); return { ok: true }; }, acknowledgeAppointmentChange: (appointmentId) => { const current = appointments.find((appointment) => appointment.id === appointmentId); if (!current?.changeMessage || current.status === "cancelled") return { ok: false, message: "There is no active appointment change to acknowledge." }; setAppointments((items) => items.map((appointment) => appointment.id === appointmentId ? { ...appointment, rescheduleAcknowledgedAt: new Date().toISOString() } : appointment)); return { ok: true }; }, recordAppointmentChangeReminderDraft: (appointmentId) => { const current = appointments.find((appointment) => appointment.id === appointmentId); const elapsed = current?.rescheduledAt ? Date.now() - Date.parse(current.rescheduledAt) : 0; if (!current?.changeMessage || current.rescheduleAcknowledgedAt || !current.rescheduledAt || elapsed < 24 * 60 * 60 * 1000) return { ok: false, message: "This appointment change is not eligible for a follow-up reminder draft." }; setAppointments((items) => items.map((appointment) => appointment.id === appointmentId ? { ...appointment, appointmentChangeReminderDraftedAt: new Date().toISOString() } : appointment)); setHistory((items) => [{ id: `history-change-reminder-${Date.now()}`, childId: current.childId, category: "Communication", title: "Appointment change acknowledgement reminder draft", occurredOn: new Date().toLocaleDateString(), note: `Clinician-reviewed reminder draft prepared for an unacknowledged updated appointment time. No message was sent automatically.` }, ...items]); return { ok: true }; },
      updateClinicHour: (weekday, changes) => setClinicHours((current) => current.map((item) => item.weekday === weekday ? { ...item, ...changes } : item)), updateServiceDuration: (service, durationMinutes) => setServices((current) => current.map((item) => item.name === service ? { ...item, durationMinutes } : item)), updateServicePreparation: (service, items) => { const cleaned = items.map((item) => item.trim()).filter(Boolean); if (!cleaned.length) return { ok: false, message: "Add at least one factual preparation item for this service." }; setPreparationChecklists((current) => current.map((checklist) => checklist.service === service ? { ...checklist, items: cleaned, updatedAt: new Date().toISOString() } : checklist)); return { ok: true }; }, updateWaitlistOfferDuration: (hours) => { if (!Number.isInteger(hours) || hours < 1 || hours > 72) return { ok: false, message: "Choose a whole-number offer duration between 1 and 72 hours." }; setWaitlistOfferDurationHours(hours); return { ok: true }; }, updateServiceWaitlistOfferDuration: (service, hours) => { if (hours !== undefined && (!Number.isInteger(hours) || hours < 1 || hours > 72)) return { ok: false, message: "Choose a whole-number service preset between 1 and 72 hours." }; setServices((current) => current.map((item) => item.name === service ? { ...item, waitlistOfferDurationHours: hours } : item)); return { ok: true }; }, requestEarlierSlot: (appointmentId, note = "") => { const current = appointments.find((appointment) => appointment.id === appointmentId); if (!current || current.status === "cancelled" || current.status === "completed") return { ok: false, message: "This appointment is no longer eligible for an earlier-slot request." }; if (earlierSlotRequests.some((request) => request.appointmentId === appointmentId && ["pending", "offered"].includes(request.status))) return { ok: false, message: "An earlier-slot request is already awaiting clinician review or has an active offer." }; setEarlierSlotRequests((currentRequests) => [{ id: `earlier-slot-${Date.now()}`, appointmentId, childId: current.childId, requestedAt: new Date().toISOString(), status: "pending", note: note.trim() || undefined }, ...currentRequests]); return { ok: true }; }, withdrawEarlierSlotRequest: (id) => { const request = earlierSlotRequests.find((item) => item.id === id); if (!request || request.status !== "pending") return { ok: false, message: "Only a pending earlier-slot request can be withdrawn." }; setEarlierSlotRequests((current) => current.map((item) => item.id === id ? { ...item, status: "withdrawn" } : item)); return { ok: true }; }, createEarlierSlotOffer: (id, date, time) => { const request = earlierSlotRequests.find((item) => item.id === id); const appointment = request ? appointments.find((item) => item.id === request.appointmentId) : undefined; if (!request || request.status !== "pending" || !appointment) return { ok: false, message: "This request is no longer available for an offer." }; const valid = validateSlot(date.trim(), time.trim(), appointment.service); if (!valid.ok) return valid; const offeredAt = new Date(); const serviceDuration = services.find((item) => item.name === appointment.service)?.waitlistOfferDurationHours ?? waitlistOfferDurationHours; const expiresAt = new Date(offeredAt.getTime() + serviceDuration * 60 * 60 * 1000).toISOString(); setEarlierSlotRequests((current) => current.map((item) => item.id === id ? { ...item, status: "offered", offer: { date: date.trim(), time: time.trim(), offeredAt: offeredAt.toISOString(), expiresAt } } : item)); return { ok: true }; }, respondToEarlierSlotOffer: (id, response) => { const request = earlierSlotRequests.find((item) => item.id === id); if (!request?.offer || request.status !== "offered") return { ok: false, message: "This earlier-slot offer is no longer active." }; if (Date.parse(request.offer.expiresAt) <= Date.now()) { setEarlierSlotRequests((current) => current.map((item) => item.id === id ? { ...item, status: "expired" } : item)); return { ok: false, message: "This earlier-slot offer has expired. Your current appointment remains unchanged." }; } setEarlierSlotRequests((current) => current.map((item) => item.id === id ? { ...item, status: "responded", offer: { ...item.offer!, parentResponse: response, respondedAt: new Date().toISOString() } } : item)); return { ok: true }; }, convertAcceptedEarlierSlotOffer: (id) => { const request = earlierSlotRequests.find((item) => item.id === id); const appointment = request ? appointments.find((item) => item.id === request.appointmentId) : undefined; if (!request?.offer || !appointment || request.status !== "responded" || request.offer.parentResponse !== "accepted") return { ok: false, message: "Only a guardian-accepted earlier-slot offer can be converted." }; if (Date.parse(request.offer.expiresAt) <= Date.now()) { setEarlierSlotRequests((current) => current.map((item) => item.id === id ? { ...item, status: "expired" } : item)); return { ok: false, message: "This accepted offer has expired and cannot be converted." }; } const valid = validateSlot(request.offer.date, request.offer.time, appointment.service, appointment.id); if (!valid.ok) return valid; const original = `${appointment.date} at ${appointment.time}`; setAppointments((current) => current.map((item) => item.id === appointment.id ? { ...item, date: request.offer!.date, time: request.offer!.time, status: "confirmed", changeMessage: `Rescheduled from ${original} after guardian accepted an earlier-slot offer.` } : item)); setEarlierSlotRequests((current) => current.map((item) => item.id === id ? { ...item, status: "converted", offer: { ...item.offer!, convertedAt: new Date().toISOString(), convertedBy: "Associate Professor Dr. Anil Ojha" } } : item)); recordAuditEvent({ id: `audit-${Date.now()}`, childId: appointment.childId, type: "appointment-change", occurredOn: "Today", actorRole: "clinician", actorName: "Associate Professor Dr. Anil Ojha", summary: `Earlier-slot offer converted: ${original} to ${request.offer.date} at ${request.offer.time}.`, message: "Guardian accepted the clinician-approved offer; clinician confirmed conversion." }); return { ok: true }; }, acknowledgeEarlierSlotResponse: (id) => setEarlierSlotRequests((current) => current.map((item) => item.id === id && item.offer?.parentResponse ? { ...item, offer: { ...item.offer, clinicianAcknowledgedAt: new Date().toISOString() } } : item)), reviewEarlierSlotRequest: (id, status) => setEarlierSlotRequests((current) => current.map((request) => request.id === id ? { ...request, status } : request)),
      addClinicBreak: (weekday, start, end) => { if (!validRange(start, end)) return { ok: false, message: "Enter a valid break range in 24-hour time, for example 12:30–13:00." }; setClinicBreaks((current) => [...current, { id: `break-${Date.now()}`, weekday, start, end }]); return { ok: true }; }, removeClinicBreak: (id) => setClinicBreaks((current) => current.filter((item) => item.id !== id)),
      addClinicHoliday: (date, label) => { if (!date.trim() || !label.trim()) return { ok: false, message: "Enter both the holiday date and a short label." }; if (clinicHolidays.some((item) => item.date === date.trim())) return { ok: false, message: "A clinic holiday already exists for that date." }; setClinicHolidays((current) => [...current, { id: `holiday-${Date.now()}`, date: date.trim(), label: label.trim() }]); return { ok: true }; }, removeClinicHoliday: (id) => setClinicHolidays((current) => current.filter((item) => item.id !== id)),
      addReferralService: (input) => { if (!input.name.trim() || !input.recipient.trim() || !input.email.trim()) return { ok: false, message: "Enter the service name, recipient, and specialist email before saving it for approval." }; setReferralServices((items) => [...items, { ...input, id: `referral-${Date.now()}`, name: input.name.trim(), recipient: input.recipient.trim(), organization: input.organization.trim(), email: input.email.trim(), defaultPurpose: input.defaultPurpose.trim(), approvalStatus: "pending" }]); return { ok: true }; }, approveReferralService: (id) => setReferralServices((items) => items.map((item) => item.id === id ? { ...item, approvalStatus: "approved", decisionBy: "Associate Professor Dr. Anil Ojha", decisionAt: new Date().toISOString() } : item)), rejectReferralService: (id) => setReferralServices((items) => items.map((item) => item.id === id ? { ...item, approvalStatus: "rejected", decisionBy: "Associate Professor Dr. Anil Ojha", decisionAt: new Date().toISOString() } : item)), removeReferralService: (id) => setReferralServices((items) => items.filter((item) => item.id !== id)), updateReferralLetterSettings: setReferralLetterSettings, updateClinicLocation: setClinicLocation, addStaffMember: (name, role) => { if (!name.trim()) return { ok: false, message: "Enter the staff member’s name before assigning a role." }; setStaffMembers((items) => [...items, { id: `staff-${Date.now()}`, name: name.trim(), role }]); return { ok: true }; }, updateStaffRole: (id, role) => setStaffMembers((items) => items.map((item) => item.id === id ? { ...item, role } : item)), removeStaffMember: (id) => setStaffMembers((items) => items.filter((item) => item.id !== id)), inviteStaffMember: (email, role) => { if (!/^\S+@\S+\.\S+$/.test(email.trim())) return { ok: false, message: "Enter a valid staff email address before creating an invitation." }; if (staffInvitations.some((item) => item.email.toLowerCase() === email.trim().toLowerCase())) return { ok: false, message: "A pending invitation already exists for that email." }; setStaffInvitations((items) => [...items, { id: `invite-${Date.now()}`, email: email.trim(), role, status: "pending" }]); return { ok: true }; }, cancelStaffInvitation: (id) => setStaffInvitations((items) => items.filter((item) => item.id !== id)),
      recordReferralLetter: (childId, recipient, purpose) => recordAuditEvent({ id: `audit-${Date.now()}`, childId, type: "referral-letter", occurredOn: "Today", actorRole: "clinician", actorName: "Associate Professor Dr. Anil Ojha", summary: `Referral letter generated for ${recipient || "receiving clinician"}.`, message: purpose.trim() || undefined }), recordReferralEmailShare: (childId, recipient, email, status, isResend = false, metadata) => { const attemptedAt = metadata?.attemptedAt ?? new Date().toISOString(); const retryAttempts = metadata?.serverRetryAttempts ?? countReferralEmailResends(auditEvents, childId, email); recordAuditEvent({ id: `audit-${Date.now()}`, childId, type: "email-share", occurredOn: "Today", actorRole: "clinician", actorName: "Associate Professor Dr. Anil Ojha", summary: `Referral email ${status.replace("-", " ")} for ${recipient || email || "selected specialist"}.`, message: email || undefined, deliveryStatus: status, isResend, attemptedAt, serverRetryLimit: metadata?.serverRetryLimit ?? MAX_REFERRAL_EMAIL_RESENDS, serverRetryAttempts: retryAttempts }); }, recordPatientCommunication: (childId, recipient, email, scope, status) => { const scopeLabel = scope === "message" ? "message" : scope === "record-pdf" ? "medical record PDF" : "timeline report PDF"; recordAuditEvent({ id: `audit-${Date.now()}`, childId, type: "patient-communication", occurredOn: "Today", actorRole: "clinician", actorName: "Associate Professor Dr. Anil Ojha", summary: `Parent ${scopeLabel} email ${status.replace("-", " ")} for ${recipient || "selected guardian"}.`, message: `${email.trim()} · ${scopeLabel}`, deliveryStatus: status, attemptedAt: new Date().toISOString() }); }, recordReminderDraft: (childId, recipient, reportLabel) => setHistory((current) => [{ id: `history-reminder-${Date.now()}`, childId, category: "Communication", title: "Report acknowledgement reminder draft", occurredOn: new Date().toLocaleDateString(), note: `Clinician-reviewed reminder draft opened for ${recipient} regarding the ${reportLabel}. No message was sent automatically.` }, ...current]),
      writePrescription: (input) => { if (!input.medication.trim() || !input.instructions.trim()) return { ok: false, message: "Add the prescription name and clinician instructions before saving." }; setPrescriptions((current) => [{ id: `rx-${Date.now()}`, ...input, issuedOn: "Today", status: "active" }, ...current]); return { ok: true }; },
    };
  }, [appointments, auditEvents, clinicBreaks, clinicDayHourOverrides, clinicHolidays, clinicHours, clinicLocation, earlierSlotRequests, history, preparationChecklists, prescriptions, referralLetterSettings, referralServices, selectedChildId, services, staffInvitations, staffMembers, waitlistOfferDurationHours]);
  return <PediatricCareContext.Provider value={value}>{content}</PediatricCareContext.Provider>;
}
export function usePediatricCare() { const context = useContext(PediatricCareContext); if (!context) throw new Error("usePediatricCare must be used within PediatricCareProvider"); return context; }
