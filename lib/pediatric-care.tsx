...
export type ReferralService = { id: string; name: string; recipient: string; organization: string; email: string; defaultPurpose: string };
export type PatientAuditEvent = { id: string; childId: string; type: "appointment-change" | "referral-letter"; occurredOn: string; actorRole: "clinician"; summary: string; message?: string };
...
export const auditEventTypesByRole: Record<StaffRole, PatientAuditEvent["type"][]> = { receptionist: ["appointment-change"], nurse: ["appointment-change", "referral-letter"], clinician: ["appointment-change", "referral-letter"] };
...
export function PediatricCareProvider({ children: content }: { children: ReactNode }) {
  ...
  const value = useMemo<PediatricCareContextValue>(() => {
    ...
    return {
      children, activeChild: children.find((child) => child.id === selectedChildId) ?? children[0], setActiveChild: setSelectedChildId, appointments, prescriptions, history, growthMetrics, clinicHours, clinicBreaks, clinicHolidays, services, referralServices, auditEvents, referralLetterSettings, staffMembers, getAvailableSlots,
      ...
      rescheduleAppointment: (appointmentId, date, time, message = "") => { const current = appointments.find((appointment) => appointment.id === appointmentId); if (!current) return { ok: false, message: "The appointment could not be found." }; const valid = validateSlot(date, time, current.service, appointmentId); if (!valid.ok) return valid; setAppointments((items) => items.map((appointment) => appointment.id === appointmentId ? { ...appointment, date, time, status: "confirmed", changeMessage: message.trim() || undefined } : appointment)); setAuditEvents((events) => [{ id: `audit-${Date.now()}`, childId: current.childId, type: "appointment-change", occurredOn: "Today", actorRole: "clinician", summary: `Appointment rescheduled to ${date} at ${time}.`, message: message.trim() || undefined }, ...events]); return { ok: true }; },
      cancelAppointment: (appointmentId, message = "") => { const current = appointments.find((appointment) => appointment.id === appointmentId); if (!current) return { ok: false, message: "The appointment could not be found." }; setAppointments((items) => items.map((appointment) => appointment.id === appointmentId ? { ...appointment, status: "cancelled", changeMessage: message.trim() || undefined } : appointment)); setAuditEvents((events) => [{ id: `audit-${Date.now()}`, childId: current.childId, type: "appointment-change", occurredOn: "Today", actorRole: "clinician", summary: "Appointment cancelled.", message: message.trim() || undefined }, ...events]); return { ok: true }; },
      ...
      addReferralService: (input) => { if (!input.name.trim() || !input.recipient.trim()) return { ok: false, message: "Enter the service name and recipient before saving it." }; setReferralServices((items) => [...items, { ...input, id: `referral-${Date.now()}`, name: input.name.trim(), recipient: input.recipient.trim(), organization: input.organization.trim(), email: input.email.trim(), defaultPurpose: input.defaultPurpose.trim() }]); return { ok: true }; }, removeReferralService: (id) => setReferralServices((items) => items.filter((item) => item.id !== id)), updateReferralLetterSettings: setReferralLetterSettings, addStaffMember: (name, role) => { if (!name.trim()) return { ok: false, message: "Enter the staff member’s name before assigning a role." }; setStaffMembers((items) => [...items, { id: `staff-${Date.now()}`, name: name.trim(), role }]); return { ok: true }; }, updateStaffRole: (id, role) => setStaffMembers((items) => items.map((item) => item.id === id ? { ...item, role } : item)), removeStaffMember: (id) => setStaffMembers((items) => items.filter((item) => item.id !== id)),
      recordReferralLetter: (childId, recipient, purpose) => setAuditEvents((events) => [{ id: `audit-${Date.now()}`, childId, type: "referral-letter", occurredOn: "Today", actorRole: "clinician", summary: `Referral letter generated for ${recipient || "receiving clinician"}.`, message: purpose.trim() || undefined }, ...events]),
  ...
  }, [appointments, auditEvents, clinicBreaks, clinicHolidays, clinicHours, prescriptions, referralLetterSettings, referralServices, selectedChildId, services, staffMembers]);
