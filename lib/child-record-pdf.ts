...
import type { ChildProfile, GrowthMetric, MedicalHistoryEntry, PediatricAppointment, PrescriptionRecord } from "@/lib/pediatric-care";
...
export function buildReferralLetterHtml(child: ChildProfile, recipient: string, purpose: string, body: string) {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><style>@page { margin: 44px; } body { font-family: Arial, sans-serif; color: #102A43; line-height: 1.55; } header { border-bottom: 3px solid #0E7490; padding-bottom: 14px; margin-bottom: 26px; } h1 { font-size: 22px; margin: 0 0 4px; } p { font-size: 12px; white-space: pre-wrap; } .label { font-size: 10px; color: #627D98; font-weight: bold; letter-spacing: .4px; } footer { border-top: 1px solid #D9E2EC; color: #627D98; font-size: 10px; margin-top: 32px; padding-top: 12px; }</style></head><body><header><p class="label">DR. ANIL OJHA CHILD CARE</p><h1>Referral Letter</h1><p><strong>${escapeHtml(child.name)}</strong> · Date of birth: ${escapeHtml(child.dateOfBirth)}</p></header><p>${escapeHtml(recipient || "To the receiving clinician")}</p><p><strong>Purpose:</strong> ${escapeHtml(purpose || "Clinical referral")}</p><p>${escapeHtml(body || "Please add the referral letter body before exporting.")}</p><p>Sincerely,\nAssociate Professor Dr. Anil Ojha\nPediatrician & Child Development Specialist</p><footer>This referral letter was prepared for clinician review. Confirm recipient, purpose, and body before using it outside the practice.</footer></body></html>`;
}
