...
export function buildWeeklyCapacitySummaryHtml(input: { internalReportId: string; reviewedBy: string; weekStartDate: string; weekEndDate: string; reviewedAt: string; unacknowledgedAlertCount: number; rows: Array<{ staffName: string; triageCapacity: number; assignmentCount: number; targetEffectiveAt: string }> }) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>@page { margin: 36px; } body { font-family: Arial, sans-serif; color: #102A43; line-height: 1.45; } header { border-bottom: 3px solid #0E7490; padding-bottom: 14px; margin-bottom: 20px; } h1 { font-size: 22px; margin: 0 0 4px; } p { font-size: 12px; margin: 4px 0; } .label { font-size: 10px; color: #627D98; font-weight: bold; letter-spacing: .4px; } table { width: 100%; border-collapse: collapse; margin-top: 16px; } th, td { border: 1px solid #D9E2EC; padding: 8px; text-align: left; font-size: 11px; } th { background: #E0F2F3; } footer { border-top: 1px solid #D9E2EC; color: #627D98; font-size: 10px; margin-top: 24px; padding-top: 12px; }</style></head><body><header><p class="label">RAINBOW CHILD DEVELOPMENT CLINIC · INTERNAL OPERATIONAL REPORT</p><h1>Reviewed Weekly Capacity Summary</h1><p><strong>Reviewed week:</strong> ${escapeHtml(input.weekStartDate)} to ${escapeHtml(input.weekEndDate)}</p><p><strong>Clinician review prepared:</strong> ${escapeHtml(new Date(input.reviewedAt).toLocaleString())}</p><p><strong>Unacknowledged capacity alerts:</strong> ${input.unacknowledgedAlertCount}</p></header><table><thead><tr><th>Staff member</th><th>Configured target</th><th>Assignments recorded</th><th>Target effective at</th></tr></thead><tbody>${rows}</tbody></table><footer><p><strong>Internal report identifier:</strong> ${escapeHtml(input.internalReportId)}</p><p><strong>Clinician review:</strong> ${escapeHtml(input.reviewedBy)} · ${escapeHtml(new Date(input.reviewedAt).toLocaleString())}</p><p>This is a clinician-approved internal tracking identifier, not a cryptographic signature, performance certification, print-completion proof, or delivery record. The report provides aggregate operational workload context only and excludes child information, appointment details, clinical content, and staff performance ratings.</p></footer></body></html>`;
}
