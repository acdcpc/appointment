...
export async function setMaintenanceMode(input: { enabled: boolean; notice: string; actorEmail: string }) {
  ...
  await db.insert(superAdminMaintenanceEvents).values({ eventId: `maintenance-${Date.now()}`, actorEmail: input.actorEmail, enabled: input.enabled, noticeSummary: input.notice, occurredAt: now }); return getSuperAdminGovernanceSettings();
}

export async function getMaintenanceModeStatus() { const settings = await getSuperAdminGovernanceSettings(); return { enabled: settings.maintenanceModeEnabled, notice: settings.maintenanceNotice, changedAt: settings.maintenanceChangedAt, changedBy: settings.maintenanceChangedBy }; }

export async function submitPostDeploymentFeedback(input: { submittedBy: string; category: "login" | "scheduling" | "records" | "display" | "other"; title: string; description: string }) {
