...
export async function setMaintenanceMode(input: { enabled: boolean; notice: string; estimatedCompletion?: string | null; actorEmail: string }) {
  ...
  await db.insert(superAdminMaintenanceEvents).values({ eventId: `maintenance-${Date.now()}`, actorEmail: input.actorEmail, enabled: input.enabled, noticeSummary: input.notice, estimatedCompletion: input.estimatedCompletion ?? null, occurredAt: now }); return getSuperAdminGovernanceSettings();
}

export async function getMaintenanceModeStatus() { const settings = await getSuperAdminGovernanceSettings(); return { enabled: settings.maintenanceModeEnabled, notice: settings.maintenanceNotice, estimatedCompletion: settings.maintenanceEstimatedCompletion, changedAt: settings.maintenanceChangedAt, changedBy: settings.maintenanceChangedBy }; }
