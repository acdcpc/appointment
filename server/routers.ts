...
export const appRouter = router({
  ...
  administration: router({
    ...
    setMaintenanceMode: superAdminProcedure.input(z.object({ enabled: z.boolean(), notice: z.string().trim().min(12).max(300), estimatedCompletion: z.string().trim().max(160).optional(), confirmed: z.literal(true) })).mutation(async ({ ctx, input }) => {
      const settings = await referralDb.setMaintenanceMode({ enabled: input.enabled, notice: input.notice, estimatedCompletion: input.estimatedCompletion || null, actorEmail: ctx.user.email ?? "super-admin" }); return { enabled: settings.maintenanceModeEnabled, notice: settings.maintenanceNotice, estimatedCompletion: settings.maintenanceEstimatedCompletion, changedAt: settings.maintenanceChangedAt?.toISOString() ?? null };
    }),
  ...
  clinicPublic: router({
    maintenanceStatus: publicProcedure.query(async () => { const status = await referralDb.getMaintenanceModeStatus(); return { enabled: status.enabled, notice: status.notice, estimatedCompletion: status.estimatedCompletion }; }),
