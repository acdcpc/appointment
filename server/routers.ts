...
import { decodeFeedbackScreenshot } from "./deployment-feedback-attachment";
...
export const appRouter = router({
  ...
  administration: router({
    ...
    setMaintenanceMode: superAdminProcedure.input(z.object({ enabled: z.boolean(), notice: z.string().trim().min(12).max(300), estimatedCompletion: z.string().trim().max(160).optional(), estimatedCompletionAt: z.string().datetime({ offset: true }).optional(), confirmed: z.literal(true) })).mutation(async ({ ctx, input }) => {
      const estimatedCompletionAt = input.estimatedCompletionAt ? new Date(input.estimatedCompletionAt) : null; if (estimatedCompletionAt && (!input.enabled || estimatedCompletionAt.getTime() <= Date.now())) throw new TRPCError({ code: "BAD_REQUEST", message: "Choose a future estimate only while maintenance mode is enabled." });
      const settings = await referralDb.setMaintenanceMode({ enabled: input.enabled, notice: input.notice, estimatedCompletion: input.estimatedCompletion || null, estimatedCompletionAt, actorEmail: ctx.user.email ?? "super-admin" }); return { enabled: settings.maintenanceModeEnabled, notice: settings.maintenanceNotice, estimatedCompletion: settings.maintenanceEstimatedCompletion, estimatedCompletionAt: settings.maintenanceEstimatedCompletionAt?.toISOString() ?? null, changedAt: settings.maintenanceChangedAt?.toISOString() ?? null };
    }),
    maintenanceNotificationPreferences: superAdminProcedure.input(z.object({ status: z.enum(["all", "requested", "withdrawn"]).optional(), emailQuery: z.string().trim().max(160).optional() })).query(async ({ input }) => (await referralDb.listMaintenanceNotificationPreferences(input)).map((preference) => ({ ...preference, maintenanceChangedAt: preference.maintenanceChangedAt.toISOString(), requestedAt: preference.requestedAt.toISOString(), updatedAt: preference.updatedAt.toISOString() }))),
    updateMaintenanceNotificationPreference: superAdminProcedure.input(z.object({ requestId: z.string().min(1).max(120), status: z.enum(["requested", "withdrawn"]), confirmed: z.literal(true) })).mutation(async ({ input }) => referralDb.setMaintenanceNotificationPreferenceStatus(input)),
    bulkUpdateMaintenanceNotificationPreferences: superAdminProcedure.input(z.object({ requestIds: z.array(z.string().min(1).max(120)).min(1).max(100).refine((ids) => new Set(ids).size === ids.length, "Select each preference only once."), status: z.enum(["requested", "withdrawn"]), confirmed: z.literal(true) })).mutation(async ({ input }) => referralDb.setBulkMaintenanceNotificationPreferenceStatus(input)),
    serviceSuggestions: superAdminProcedure.input(z.object({ status: z.enum(["all", "submitted", "approved", "dismissed"]).optional(), serviceQuery: z.string().trim().max(80).optional() })).query(async ({ input }) => (await referralDb.listServiceSuggestions(input)).map((suggestion) => ({ ...suggestion, requestedAt: suggestion.requestedAt.toISOString(), reviewedAt: suggestion.reviewedAt?.toISOString() ?? null, updatedAt: suggestion.updatedAt.toISOString() }))),
    reviewServiceSuggestion: superAdminProcedure.input(z.object({ suggestionId: z.string().min(1).max(120), status: z.enum(["approved", "dismissed"]), confirmed: z.literal(true) })).mutation(async ({ ctx, input }) => referralDb.reviewServiceSuggestion({ ...input, reviewedBy: ctx.user.email ?? "super-admin" })),
    prepareMaintenanceNotificationPreferenceCsv: superAdminProcedure.input(z.object({ status: z.enum(["all", "requested", "withdrawn"]).optional(), emailQuery: z.string().trim().max(160).optional(), confirmed: z.literal(true) })).mutation(async ({ ctx, input }) => { const rows = await referralDb.prepareMaintenanceNotificationPreferenceExport({ ...input, actorEmail: ctx.user.email ?? "super-admin" }); return { rows: rows.map((preference) => ({ ...preference, maintenanceChangedAt: preference.maintenanceChangedAt.toISOString(), requestedAt: preference.requestedAt.toISOString(), updatedAt: preference.updatedAt.toISOString() })), disclaimer: "This export contains user-entered contact preferences. Preparation is recorded; it does not send an email or prove delivery, viewing, retention, or disposal." }; }),
    postDeploymentFeedback: superAdminProcedure.query(async () => (await referralDb.listPostDeploymentFeedback()).map(({ screenshotStorageKey, screenshotContentType, screenshotBytes, ...item }) => ({ ...item, screenshotAvailable: Boolean(screenshotStorageKey), screenshotContentType: screenshotStorageKey ? screenshotContentType : null, screenshotBytes: screenshotStorageKey ? screenshotBytes : null, submittedAt: item.submittedAt.toISOString(), reviewedAt: item.reviewedAt?.toISOString() ?? null, updatedAt: item.updatedAt.toISOString() }))),
    getPostDeploymentFeedbackScreenshot: superAdminProcedure.input(z.object({ feedbackId: z.string().min(1).max(120), acknowledged: z.literal(true) })).query(async ({ input }) => { const attachment = await referralDb.getPostDeploymentFeedbackAttachment(input.feedbackId); if (!attachment?.screenshotStorageKey) throw new TRPCError({ code: "NOT_FOUND", message: "No protected screenshot is attached to this feedback record." }); return { url: await storageGetSignedUrl(attachment.screenshotStorageKey), contentType: attachment.screenshotContentType, bytes: attachment.screenshotBytes, meaning: "Temporary super-admin review access only. The attachment is unverified feedback evidence and does not prove an incident or resolution." }; }),
    ...
    searchAppointmentExportRegister: superAdminProcedure.input(z.object({ startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), actorQuery: z.string().trim().max(160).optional() })).query(async ({ input }) => {
      ...
      const events = await referralDb.searchSuperAdminAppointmentExportRegister(input);
      return events.map((event) => ({ ...event, occurredAt: event.occurredAt.toISOString(), disclaimer: "Preparation evidence only; this register does not prove download, delivery, viewing, retention, or disposal." }));
    }),
    prepareAppointmentCsv: superAdminProcedure.input(z.object({ startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/) })).mutation(async ({ ctx, input }) => {
      ...
      const exportResult = await referralDb.prepareSuperAdminAppointmentExport({ actorEmail: ctx.user.email ?? "super-admin", ...input });
      return { appointments: exportResult.rows.map((appointment) => ({ appointmentId: appointment.appointmentId, clinicianUserId: appointment.clinicianUserId, childId: appointment.childId, service: appointment.service, appointmentDate: appointment.appointmentDate, appointmentTime: appointment.appointmentTime, durationMinutes: appointment.durationMinutes, reason: appointment.reason, status: appointment.status, changeMessage: appointment.changeMessage, guardianConfirmedAt: appointment.guardianConfirmedAt?.toISOString() ?? null, rescheduledAt: appointment.rescheduledAt?.toISOString() ?? null, createdAt: appointment.createdAt.toISOString(), updatedAt: appointment.updatedAt.toISOString() })), retentionDays: exportResult.retentionDays, retentionExpiresAt: exportResult.retentionExpiresAt.toISOString(), confidentialityNotice: "Confidential appointment record export. Preparation is audited and does not prove download, delivery, secure storage, or disposal." };
    }),
  }),
  clinicPublic: router({
    maintenanceStatus: publicProcedure.query(async () => { const status = await referralDb.getMaintenanceModeStatus(); return { enabled: status.enabled, notice: status.notice, estimatedCompletion: status.estimatedCompletion, estimatedCompletionAt: status.estimatedCompletionAt?.toISOString() ?? null }; }),
    ...
    suggestService: publicProcedure.input(z.object({ suggestedService: z.string().trim().min(2).max(80).refine((value) => /\S/.test(value), "Enter a service name."), notificationEmail: z.string().trim().email().max(320).optional(), notificationConsented: z.boolean() }).refine((value) => Boolean(value.notificationEmail) === value.notificationConsented, "Enter an email only when you explicitly request review of a future availability notice.")).mutation(async ({ input }) => {
      const suggestion = await referralDb.submitServiceSuggestion(input); return { ...suggestion, meaning: "Your non-clinical service suggestion was recorded for later super-admin review. An optional email preference does not send an email automatically, guarantee contact, or promise that the clinic will add the service." };
    }),
  ...
  clinician: router({
    ...
    submitPostDeploymentFeedback: clinicAdminProcedure.input(z.object({ category: z.enum(["login", "scheduling", "records", "display", "other"]), title: z.string().trim().min(5).max(140), description: z.string().trim().min(12).max(1200), screenshot: z.object({ base64: z.string().regex(/^[A-Za-z0-9+/=]+$/), contentType: z.enum(["image/png", "image/jpeg", "image/webp"]) }).optional() })).mutation(async ({ ctx, input }) => {
      let screenshot: { screenshotStorageKey?: string; screenshotContentType?: string; screenshotBytes?: number } = {};
      if (input.screenshot) { let decoded; try { decoded = decodeFeedbackScreenshot(input.screenshot.base64, input.screenshot.contentType); } catch (error) { throw new TRPCError({ code: "BAD_REQUEST", message: error instanceof Error ? error.message : "The screenshot could not be processed." }); } const extension = input.screenshot.contentType === "image/png" ? "png" : input.screenshot.contentType === "image/jpeg" ? "jpg" : "webp"; const uploaded = await storagePut(`deployment-feedback/${randomUUID()}.${extension}`, decoded.bytes, input.screenshot.contentType); screenshot = { screenshotStorageKey: uploaded.key, screenshotContentType: input.screenshot.contentType, screenshotBytes: decoded.bytes.length }; }
      const feedbackId = await referralDb.submitPostDeploymentFeedback({ category: input.category, title: input.title, description: input.description, submittedBy: ctx.user.email ?? "clinic-administrator", ...screenshot }); return { feedbackId, submitted: true, screenshotAttached: Boolean(screenshot.screenshotStorageKey), meaning: "Feedback was recorded for super-admin review. Submission does not resolve the issue or send a message automatically." };
    }),
    ...
    reorderInvitationSearchPresets: adminProcedure.input(z.object({ presetIds: z.array(z.string().min(1).max(120)).max(24) })).mutation(async ({ ctx, input }) => { await referralDb.reorderInvitationSearchPresets(ctx.user.id, input.presetIds); return { reordered: true }; }),
    ...
    listWeeklyCapacityReportReferenceExpiryReminders: adminProcedure.query(async ({ ctx }) => (await referralDb.listWeeklyCapacityReportReferenceExpiryReminders(ctx.user.id)).map((report) => ({ internalReportId: report.internalReportId, weekStartDate: report.weekStartDate, weekEndDate: report.weekEndDate, referenceExpiresAt: report.referenceExpiresAt?.toISOString() ?? null }))),
