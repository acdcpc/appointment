...
function ClinicianLogin({ onBack, onSignedIn }: { onBack: () => void; onSignedIn: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!isClinicAdministratorEmail(result.user.email ?? "")) {
      ...
      else setError(raw || "Sign-in failed. Please try again.");
    } finally {
      setSigning(false);
    }
  };
  return <ScreenContainer className="p-5"><View style={styles.loginWrap}><Pressable onPress={onBack}><Text style={[styles.back, { color: colors.primary }]}>‹  Back</Text></Pressable><View style={[styles.loginCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={[styles.lock, { backgroundColor: colors.tealSurface }]}><Text style={{ color: colors.primary, fontWeight: "800", fontSize: 22 }}>⌁</Text></View><Text style={[styles.loginTitle, { color: colors.foreground }]}>Clinic team sign in</Text><Text style={[styles.loginText, { color: colors.muted }]}>Use an approved clinic account. Clinician administrators manage records; activated staff members can open only their assigned operational workspace.</Text><Text style={[styles.label, { color: colors.muted }]}>Email</Text><TextInput value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" placeholder="doctor@clinic.com" placeholderTextColor={colors.muted} style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.background }]} accessibilityLabel="Clinic account email" /><Text style={[styles.label, { color: colors.muted }]}>Password</Text><TextInput value={password} onChangeText={setPassword} secureTextEntry style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.background }]} accessibilityLabel="Clinic account password" />{error ? <Text style={{ color: colors.error, fontWeight: "800", fontSize: 13 }}>{error}</Text> : null}<Pressable onPress={submit} disabled={signing || !emailValid || password.length < 6} style={[styles.primaryButton, { backgroundColor: colors.primary, opacity: signing || !emailValid || password.length < 6 ? 0.6 : 1 }]} accessibilityRole="button" accessibilityLabel="Sign in to the clinic dashboard">{signing ? <ActivityIndicator color={colors.textInverse} /> : <Text style={[styles.primaryButtonText, { color: colors.textInverse }]}>Sign in securely</Text>}</Pressable><Text style={[styles.loginFootnote, { color: colors.muted }]}>Access is assigned by the clinician administrator and verified after sign-in.</Text></View></View></ScreenContainer>;
}
...
function Dashboard({ focus, reportId }: { focus?: "contacts" | "staff" | "email-shares"; reportId?: string }) {
  const colors = useColors(); const router = useRouter(); const scrollRef = useRef<ScrollView>(null); const [sectionOffsets, setSectionOffsets] = useState<Record<string, number>>({}); const { appointments, children, activeChild, clinicHours, clinicBreaks, clinicHolidays, services, updateClinicHour, updateServiceDuration, addClinicBreak, removeClinicBreak, addClinicHoliday, removeClinicHoliday, writePrescription } = usePediatricCare();
  ...
  const scheduledQueue = useMemo(() => appointments.filter((appointment) => appointment.status !== "completed"), [appointments]);
  useEffect(() => { if (!selectedAppointment && appointments[0]) setSelectedAppointment(appointments[0].id); }, [appointments, selectedAppointment]);
  ...
  const saveSectionOffset = (section: string, offset: number) => setSectionOffsets((current) => current[section] === offset ? current : { ...current, [section]: offset });
  const jumpToSection = (section: string) => { const offset = sectionOffsets[section]; if (typeof offset === "number") scrollRef.current?.scrollTo({ y: Math.max(0, offset - 8), animated: true }); };
  const shortcuts = [{ id: "appointments", label: "Schedule" }, { id: "operations", label: "Operations" }, { id: "care", label: "Care tools" }, { id: "settings", label: "Settings" }];
  return <ScreenContainer className="p-5"><ClinicianOperationalSync /><ClinicianAppointmentSync /><ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
